...
- The shifted Gamma distribution, $\text{ShiftedGamma}(\mu, \beta; \epsilon)$, is a standard Gamma distribution with mean $\mu$ and scale $\beta$ that is shifted to the right by $\epsilon$, with the support $[\epsilon, \infty)$. 
...
All nine scenarios (s1-s9) were analyzed using $4$ chains with only one replicate. 
...
## Data Application, Evaluation Results
