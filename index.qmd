...
In all nine scenarios, we vary $P$ and $\epsilon$, resulting in six cases: 
...
For brevity, we will focus on Scenario 1.
...
```{r}
table_combind <- readRDS(file.path(output_dir, "table_combind.rds"))
table_combind <- table_combind |> kable(digits = 4)
table_combind
```
...
- The overall coverage can obscure important differences in predictive performance.

- To provide a clearer picture, we decompose the coverage into **below** and **above**. 
...
- a prior study found comparable performance between the probabilistic MTD model and the deep learning long short-term memory (LSTM) network for predicting disease spread.
...
> **Objectives:** Systemically evaluate LSTM and MTD models to ensure a fair and balanced comparison of their performance and robustness. 
