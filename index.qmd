...
As $t$ increases, the size of the conditioning set, $\textbf{X}^{t-1}$, can be large. 
...
For brevity, we present data application only. 

# Data Application 

## $95\%$ Predictive Intervals, One-Step Ahead; Full Test Size $n = 1756$ 
...
In all nine scenarios (s1-9), we vary $P$ and $\epsilon$, resulting in six cases per scenario: 
...
[^coveragezigamma]: Each row label indicates the combination of $P$ (zero-inflated probability) and $\epsilon$ (threshold value) used in the simulation. For example, P01eps01 reads as $P = 0.1$ and $\epsilon = 0.1$. 
...
## $95\%$ Predictive Intervals, One-Step Ahead; Full Test Size $n = 1756$; MTD Only [^captionmtd]
...
- This extension again demonstrates the framework’s generalizability beyond the Gamma marginals.

## Conclusion

Part III

Through simulation studies and data application, we illustrate how MTD and LSTM serve as complementary approaches for modeling complex, skewed, and zero-inflated time series. 
...
Overall, our proposed MTD framework provides a flexible, robust, and interpretable approach for modeling complex continuous and zero-inflated time series, demonstrating superior performance compared to LSTM in the settings considered. 
...
- Although the current framework assumes stationarity and does not incorporate covariates/features, it can be readily extended to enhance flexibility and realism in modeling complex temporal patterns.

- We use MCMC as our primary sampling method, but alternative approaches such as Variational Inference or Sequential Monte Carlo could be explored.
...
Thank you to `Lisa Madsen` and `Charlotte Wickham`, for your dedication, support, and constant inspiration throughout the process. 
...
Thank you to `my family`, my mom, dad, and brother, for your financial support and for patience and understanding througout the journey.
...
-- (2023a). “Bayesian geostatistical modeling for discrete-valued processes”, *Environmetrics*, Vol. 425 34 No. 7, e2805. 

-- (2023b). “Nearest-neighbor mixture models for non-Gaussian spatial processes”, *Bayesian Analysis*, Vol. 18 No. 4, pp. 1191–1222.
