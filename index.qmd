...
- ⚠️ Claims of LSTM superiority can be misleading with inappropriate benchmarks. 

. . .    

- ⚠️ A prior work found comparable performance between probabilistic Mixture Transition Distribution (MTD) model and deep learning LSTM in, e.g., predicting disease spread.
...
> Develop the copula-based Gamma Mixture Transition Distribution (Gamma MTD) model and its zero-inflated extension (ZIGamma MTD), effectively capture high-order dependence, skewed behavior, and semicontinuous patterns. 

. . .

Part III of the dissertation

> Evaluate the proposed models against the Long Short-Term Memory (LSTM) networks, achieving higher predictive accuracy, while providing uncertainty quantification (UQ) and interpretability. 
...
- (Skip) Training, Hyperparameter Tuning, and Metrics 
...
The mixture transition distribution (MTD) models is a flexible extension of autoregressive (AR) models

- Accommodate non-Gaussian and nonlinear features [^nonlinear]. 

[^nonlinear]: E.g., flat stretches, bursts, outliers, and change points. 

. . .    

- Use a state-space approach based on Markov chains. 

. . .    

- Represent high-order dependence as a mixture of first-order transition kernels with stochastic weights. 
...
- Includes various applications, extending beyond linear, Gaussian dynamics [^gaussian]. 

[^gaussian]: E.g., Gaussian mixture, Poisson, Negative Binomial, and Lomax regression MTD models. 
...
There are two components in (2):
  ...
  - Transition kernel [^kernel], $f_l$, captures the influence of the $l$th lag value on the current value. 
  
  - Weight parameter [^weight], $w_l$, determines the relative contribution of that influence. 

[^kernel]: Transition kernel generalizes transition probabilities to a more general state space.

[^weight]: See Supplement. 
...
Copula provide a flexible framework for dependence modeling.

  - Widely used in quantitative finance, reliability engineering, hydrology, etc.
...
Copula 
  ...
  - is a multivariate cumulative distribution function (cdf) with $\text{Unif}(0, 1)$ marginals. 

. . .

  - Any joint distribution, $F$, can be decomposed into two parts: a copula, $C$, and the marginals, $F_j$, $j = 1,..., p$. 

    - Rooted in Sklar’s theorem [^sklar].

. . .

- Families of copula offer a wide range of dependence structures, constituting an active research area.
...
- identifies a bivariate distribution of $(U_l, V_l)$ such that the marginal densities, $f_{U_l}$ and $f_{V_l}$, are equal to a pre-specified stationary marginal density $f_X$ for all $l$ transition kernels. 

  - Based on Proposition 1 [^prop1] of Zheng et al. (2022).

. . .

The construction of such transition kernel can be challenging or lacks a simple explicit form.

. . .

  - Solution: Use a copula to construct the transition kernel with the desired density.
...
Earlier MTD models were built through frequentist approaches and estimated via EM algorithms. 

. . .

Zheng et al. (2022) develope the Bayesian MTD model and estimated via Markov chain Monte Carlo (MCMC) algorithms. 

- Our work: Copula-based extension of the Bayesian MTD framework. 
...
With priors specifications [^prior], the full simulation-based Bayesian estimation and prediction can be achieved using MCMC algorithms. 
...
- Generate $n = 2,000$ observations from the copula-based Gamma MTD model.
...
The model performs well across these evaluations, but we are not gonna discuss them here [^sen].
...
Zero-inflated data are characterized by an excess of zero values and 

- frequently appear in domains such as healthcare, insurance, environment, and transportation. 
...
- Has been applied in discrete and spatial settings but not in our specific setting. 
...
## Simulation Results [^resmar] [^resmar2]
...
[^resmar]: There is no evidence of lack of convergence for all parameters (Gelman-Rubin statistic $R$ and its upper CI $\leq$ $1.1$). 

[^resmar2]: In our latest model version, $\epsilon$ is fixed at a data-driven value rather than estimated. 
...
- Most parameters achieve full coverage [^coveragedef], with a few slightly below $1$, indicating that the posterior intervals reliably capture the true parameter values. 

[^coveragedef]: The overall coverage is the proportion of replicates for which the true value is contained within the interval. 
...
[^coveragezigamma]: Each row name indicates the combination of $P$ (zero-inflated probability) and $\epsilon$ (threshold value). For example, P01Eps01 reads as $P = 0.1$ and $\epsilon = 0.1$. 
...
Recurrent Neural Networks (RNNs), and their variants, Long Short-Term Memory (LSTMs), are 

- widely used for modeling sequence data because of their ability to capture short- and long-term dependencies.

. . .

Beyond natural language processing (NLP), 

- they have also shown effective in time series forecasting and 

. . .

- have been employed for applications including
...
- previous studies comparing LSTMs to traditional models such as ARIMA often claim LSTM superiority, which can be misleading when the benchmarks chosen are inappropriate.
...
> Demonstrate that our proposed MTDs outperform LSTMs in accuracy, with UQ and interpretability, though at higher computational cost and with more involved model design. 
...
- number of hidden units or cell dimension, and
...
![](images/windpower.jpg){.absolute top=0 left=0 width="1200" height="675"}
...
- Wind speed is a key proxy variable for energy forecasting. 

  - Accurate prediction of wind speed is crucial for reliable power generation.   

. . . 

- Modeling wind speed is challenging. 

  - Strong temporal dependence and skewed distribution, which can pose challengings for conventional modeling approaches. 

. . . 

Goal 

- Produce accurate forecasts and evaluate the strengths and limitations of MTD and LSTM for capturing temporal dependence and skewness. 
...
- Hourly wind speeds (m/s) at 50 m, 10 m, and 2 m above ground level at the Limon Wind Energy Center from 2024-2025. 

  - Accessed via the NASA GES DISC Earthdata API. 

  - Derived wind speeds from U/V wind components. 

## Data Processing 

Feature Engineering

- Created lagged features to capture temporal dependence [^limitation]. 

  - Tested different look-back windows, e.g., $L = 5, 15$. 

. . . 

Train/Test Split

- Used an 80/20 chronological split, with the first $80\%$ of observations used for training and the final $20\%$ used for testing to preserve the temporal ordering. 

. . . 

LSTM Only

- Applied min-max scaling for input features and labels, and inverse-transformed predictions back to the original scale for improved stability. 

[^limitation]: The current MTD formulation does not support additional predictors, so neither model includes external predictors. This also provides a simple baseline for isolating the impact of added features and model complexity. 

## Modeling Approach: MTD vs LSTM

MTD (Mixture Transition Distribution)

- Bayeisan probabilistic model 

- Explicitly models marginal distribution and temporal dependence through a weighted mixture of transition kernels 

- Gamma marginal + Gaussian copula

. . . 

LSTM (Long Short-Term Memory)

- Neural-network-based recurrent model

- Learns temporal dependencies directly from data
...
- For MTD, the model specifications were:

  - marginal distribution: Gamma
...
- For LSTM, the architecture consisted of: 

  - standard LSTM with forget gate, implemented in PyTorch.
...
- The hyperparameter settings were guided by prior experimentation and coverage analysis:
...
[^alpha]: $u_{\alpha}$, $u_{\beta}$ derived from the training data: $u_{\beta}$ = mean/variance, $u_{\alpha}$ = mean $\times$ $u_{\beta}$. $u$ and $v$ can be further tuned to control the strength of the prior. 

[^w]: $\alpha_0$: concentration parameter; $G_0$: base distribution, i.e., $G_0 \sim Beta(a_0, b_0)$, where $a_0$ and $b_0$ are the shape parameters.  
...
- The configurations were chosen based on a comprehensive literature review and our experiments: 
...
- The MCMC settings were:
  ...
  - number of iterations: $85,000$ 

  - burn-in: $5,000$, and 

  - thinning: $20$. 

- MTD is designed and built in `R` and `C++`, then compiled and executed on a high-performance computing (HPC) cluster. 
...
- LSTM was trained using: 

  - Adam optimizer with a learning rate of $0.001$. 

  - L1 loss for MAE and L2 loss for all other metrics 🔜. 

- LSTM is implemented with `PyTorch` and trained on a standard workstation.
...
[^datares2]: LSTM is trained using L1 loss for MAE; L2 loss for all other metrics. MTD does not rely on training with an explicit loss function. 

[^datares]: MASE $= 1$ serves as the naïve benchmark; MASE $< 1$ indicates better performance. 
...
[^captiondiff]: Solid (red) lines show differences between LSTM predicted means and observed values and solid (blue) lines show differences between MTD predicted means and observed values.

[^captiondiff2]: See Supplement for additional plots. 

## $95\%$ Predictive Intervals, One-Step Ahead [^predcoverage]; Full Test Size $n = 1756$; MTD Only
...
[^predcoverage]: Coverage is the proportion of test observations falling within their corresponding $95\%$ predictive intervals. 

## $95\%$ Predictive Intervals, One-Step Ahead [^predcoverageplot]; Full Test Size $n = 1756$; MTD Only 
...
[^predcoverageplot]: Solid (black) line shows test observed values and shaded (purple) band shows one-step ahead $95\%$ predictive intervals.  

## Takeaway

Results 

- MTD consistently outperformed LSTM across all evaluation metrics and heights.

  - Probabilistic formulation provided uncertainty quantification and model interpretability. 

- LSTM performed *competitively* and was computationally efficient. 

. . . 

Trade-offs

- MTD requires careful model design and is more computationally intensive.

- LSTM is more general-purpose, but less interpretable due to its black-box structure. 

. . . 

Bottom-line

> MTD achieved better predictive performance and provided UQ and interpretability, while LSTM offered a more general-purpose and computationally efficient alternative. 

## Next Steps

Further Experiments & Analysis

  - Complete Transformer experiments and compare performance with MTD and LSTM on the current dataset.

  - Evaluate LSTM and Transformer on the zero-inflated dataset.

. . . 

MTD Extensions [^mtdimprove]

  - Incorporate external predictors to relax the stationarity assumption.

  - Explore alternative marginal distributions and copula choices, as well as more efficient MCMC methods.

. . . 

ML/DL UQ and Interpretability [^lstmimprove]

  - Explore probabilistic/quantile approaches for UQ and post-hoc methods for interpretability of LSTM and Transformer forecasts. 

[^mtdimprove]: The current model uses Gamma marginals and a Gaussian copula, but the general MTD framework can accommodate other distributions (e.g., Beta, Pareto), copulas (e.g., Clayton, Gumbel), and more efficient MCMC methods (e.g., variational inference, sequential Monte Carlo). 

[^lstmimprove]: Alternative loss functions, such as quantile loss, can also be used to train LSTMs to produce predictive uncertainty in addition to point forecasts. 

## Bonus: MTD Extension to Zero-Inflated Data [^rain] [^predcoverageplot]

::: {.panel-tabset}

### P(zero) = 0.34; precipitation at Quibdó, Colombia
...
```{r}
readRDS(file.path(output_dir, "p2_real_rain.rds"))
```

### P(zero) = 0.69; precipitation at Medan, North Sumatra

```{r}
readRDS(file.path(output_dir, "p4_real_rain.rds"))
```

::: 

[^rain]: We initially planned to use insurance claims data, but such data are often private. The model could potentially be applied to time series of insurance claims per policyholder or hospital stays per patient. 
...
We further develop a copula-based zero-inflated MTD model, which preserves the advantages of copula modeling for capturing dependence in zero-inflated continuous settings. 

. . .    

> **Key contribution:** A unified MTD framework that accommodates complex dependence modeling and diverse marginal distributions. 
...
- MTD models are more robust and interpretable, but require greater computational time and more involved model design. 

- LSTMs are general-purpose and computationally efficient, but their black-box nature limits interpretability and may lead to lower accuracy. 

. . .    

> **Overall impact:** A flexible, robust, and interpretable framework for forecasting complex zero-inflated, skewed time series (i.e., rare, tail events), outperfoming LSTMs in the settings considered. 
...
- Future work should include alternative copulas, like Clayton and Gumbel, to capture tail dependence and asymmetry. 

. . .  

- Future work could incorporate Beta, Lognormal, and Weibull distributions and their ZI versions to accommodate diverse data characteristics and applications. 

. . .  

- Although current framework assumes stationarity and does not incorporate external predictors, it can be readily extended by introducing additional model terms in the conditional mean structure. 

. . .  

- We use MCMC as the primary estimation method. Alternative approaches such as Variational Inference or Sequential Monte Carlo could provide additional computational advantages. 

## Future Work

Part III

- Given the growing prominence of transformers in sequence modeling, further research should extend performance comparisons to include transformer-based models [^transformer] [^transformer2]. 

[^transformer]: E.g., TimeGPT-1 by Nixtla, TimesFM by Google Research, Tiny Time Mixers by IBM Research, and Chronos by AWS AI Labs. 

[^transformer2]: Preliminary results from a working transformer implementation show that it does not outperform MTD nor LSTM. 
...
- Future work could explore integrated approaches that combine probabilistic models with neural architectures, leveraging the strengths of both frameworks. 
...
Special thanks to `my partner`, Joseph, for your timely distractions and cute annoyances. 
...
## Prediction Errors [^captiondiff], One-Step Ahead; Full Test Size $n = 1756$  
...
```{r}
readRDS(file.path(output_dir, "p1_real_diff_full.rds"))
```

### windspeeds at 10m above ground

```{r}
readRDS(file.path(output_dir, "p2_real_diff_full.rds"))
```

### windspeeds at 2m above ground

```{r}
readRDS(file.path(output_dir, "p3_real_diff_full.rds"))
```
